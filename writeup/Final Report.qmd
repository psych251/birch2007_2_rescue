...
library(tidyverse); library(ggplot2); library(car); library(effsize); library(qualtRics); library(readxl); library(patchwork); library(ggpubr); library(metafor); library(RCurl); library(here)
...
*Given that raw data is not shared in the repository (due to containing identifiers),
please skip steps 1 and 2.*
...
final_data_tidy <- read.csv(here("data/final_data/2023-12-12_final_data_tidy.csv"))
...
borchers_replication_all_participants <- read.csv(here("data/borchers_replication_anonymized.csv"))
...
raw_metaanalysis_data <- read_xlsx(here("data/Birch-Bloom_minimeta_data.xlsx"), col_names = T, range = "B2:G6")
